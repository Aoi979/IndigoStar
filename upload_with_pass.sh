...
upload_password="${INDIGO_STAR_UPLOAD_PASSWORD:-}"
...
Environment defaults (all optional if provided via options):
  ...
  INDIGO_STAR_UPLOAD_PASSWORD    SSH password (used via sshpass)
...
Examples:
  ...
  INDIGO_STAR_UPLOAD_PASSWORD=yourpassword \
    ./upload_with_pass.sh
...
if [[ -z "$upload_password" ]]; then
  echo "Error: password is not set. Set INDIGO_STAR_UPLOAD_PASSWORD." >&2
  usage >&2
  exit 2
fi
