...
__global__ static __launch_bounds__(decltype(size(
    TiledMma{}))::value) void cute_ampere_hgemm_16816_no_reg_prefetch(
    ProblemShape shape_MNK, CtaTiler cta_tiler, TA const *A, AStride dA,
    ASmemLayout sA_layout, TiledCopyA copy_a, S2RAtomA s2r_atom_a,
    TB const *B, BStride dB, BSmemLayout sB_layout, TiledCopyB copy_b,
    S2RAtomB s2r_atom_b, TC *C, CStride dC, CSmemLayout sC_layout,
    TiledMma mma, Alpha alpha, Beta beta, int kBlockSwizzle) {
  ...
  while (k_tiles_to_compute > 0) {
    // No register prefetch: wait for the current smem pipe, then load the
    // current k-block into registers immediately before its MMA.
